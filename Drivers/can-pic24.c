...
// Buffer for CAN1
   ...
   #locate ecan1_message_buffer = 0x1200  //changed this address incase code is being debugged
  #endif
#endif
...
// Buffer for CAN2
      ...
      uint16_t ecan2_message_buffer[32][8];
         ...
         (getenv("DEVICE") != "DSPIC33EP256MU806") && (getenv("DEVICE") != "DSPIC33EP256MU810") && (getenv("DEVICE") != "DSPIC33EP256MU814") && \
         (getenv("DEVICE") != "DSPIC33EP512GP806") && (getenv("DEVICE") != "DSPIC33EP512MC806") && (getenv("DEVICE") != "DSPIC33EP512MU810") && (getenv("DEVICE") != "DSPIC33EP512MU814")
      #locate ecan2_message_buffer = 0x1400  //changed this address incase code is being debugged
     #endif
   #endif
...
int1 can_getd(uint32_t &id, uint8_t *data, uint8_t &len, struct rx_stat &stat)
{
   ...
   //This change was made to workaround an errata the dsPIC33EV family has
   if(stat.buffer < 16)
      (uint16_t)C1RXFUL1 = ~(0x0001 << (uint16_t)stat.buffer);
   else
      (uint16_t)C1RXFUL2 = ~(0x0001 << ((uint16_t)stat.buffer - 16));
   ...
   int1 can2_getd(uint32_t &id, uint8_t *data, uint8_t &len, struct rx_stat &stat)
   {
      ...
      //This change was made to workaround an errata the dsPIC33EV family has
      if(stat.buffer < 16)
         (uint16_t)C2RXFUL1 = ~(0x0001 << (uint16_t)stat.buffer);
      else
         (uint16_t)C2RXFUL2 = ~(0x0001 << ((uint16_t)stat.buffer - 16));
