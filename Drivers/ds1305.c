...
#define __DS1305_C__
   ...
   #use spi(master, mode=0, LSB_FIRST, do=RTC_IO, clk=RTC_SCLK, bits=8, baud=600000, stream=STREAM_DS1305_SPI_OUT)
   #use spi(master, mode=0, LSB_FIRST, di=RTC_IO, clk=RTC_SCLK, bits=8, baud=600000, stream=STREAM_DS1305_SPI_IN)
   
   #define write_ds1305_byte(x)  spi_xfer(STREAM_DS1305_SPI_OUT, x)
   #define DS1305_SPI_XFER(x)    spi_xfer(STREAM_DS1305_SPI_IN, x)
  #else
...
void rtc_init() 
{
   ...
   output_drive(RTC_SCLK);
   
  #ifdef RTC_IO
   output_drive(RTC_IO);
  #else
   output_drive(RTC_MOSI);
  #endif
