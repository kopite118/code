...
#include <stdbool.h>
   ...
   typedef bool uart1_rx_size_t;
#elif (UART1_RX_BUFFER_SIZE >= 0x100)
   typedef uint16_t uart1_rx_size_t;
#else
   typedef uint8_t uart1_rx_size_t;
#endif
...
// in the receive buffer than can be read with UART1Getc().
