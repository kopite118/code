...
// include the CAN drivers
   ...
   #define j1939can_getd   can_fifo_getd
  #endif  
 #elif defined(__PCH__)
  #include <can-18F4580.c>    //PIC18
  #define j1939can_getd   can_fifo_getd
 #else
...
void J1939Init(void)
{
   ...
   can_init();    //Initialize the CAN, sets up Baud Rate and puts it in normal mode
      ...
      can_associate_filter_to_mask(ACCEPTANCE_MASK_1, F2BP);
      
      can_enable_filter(RXF0EN | RXF1EN | RXF2EN);      
...
void J1939ReceiveTask(void)
{
   ...
   while(can_kbhit())
   {
      j1939can_getd(ReceivedPDU,Data,length,Status);
...
void J1939XmitTask(void)
{
   ...
   while((g_J1939Flags.XmitBufferCount > 0) && can_tbe())
   {
          ...
          (g_J1939XmitBuffer[g_J1939XmitNextOut].Data[1] == 0xEE) && (g_J1939XmitBuffer[g_J1939XmitNextOut].Data[2] == 0x00)))
      {
         ...
         can_putd((uint32_t)g_J1939XmitBuffer[g_J1939XmitNextOut].PDU,g_J1939XmitBuffer[g_J1939XmitNextOut].Data,g_J1939XmitBuffer[g_J1939XmitNextOut].Length,3,TRUE,FALSE);
