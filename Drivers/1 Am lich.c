...
#define BEGINNING_YEAR	18
const int16 LUNAR_CALENDAR_LOOKUP_TABLE[] = 
{
//!
//!	//2000
...
//======================================== 
void tro_bag_lunar(int16 dl)
{
	ngay_am_lich_dau_thag_duog_lich=dl&0x001f; // 5b
	dl>>=5;
	thag_am_lich_dau_thag_duog_lich=dl&0x000f; // 4b
	dl>>=4;
	so_ngay_cua_thag_am_lich_dau_thag_duog_lich=dl&0x0001;
	dl>>=1;
	thag_nhuan_bang_thag_hien_tai=dl&0x0001;
	dl>>=1;
	so_ngay_cua_thag_duog_lich=dl;//&0x0003;
}
//======================================== 
void Solar2Lunar()
{ 	
	...
	tro_bag_lunar(LUNAR_CALENDAR_LOOKUP_TABLE[(nam-BEGINNING_YEAR)*12+thang -1]);
	N_AL_DT_DL = ngay_am_lich_dau_thag_duog_lich;
	T_AL_DT_DL = thag_am_lich_dau_thag_duog_lich;
	SN_CT_AL = so_ngay_cua_thag_am_lich_dau_thag_duog_lich + 29;
	TN_B_THT =  thag_nhuan_bang_thag_hien_tai;
  	
	tro_bag_lunar(LUNAR_CALENDAR_LOOKUP_TABLE[(nam-BEGINNING_YEAR)*12+thang]);
	N_AL_DT_DL_TT = ngay_am_lich_dau_thag_duog_lich;
	T_AL_DT_DL_TT = thag_am_lich_dau_thag_duog_lich;
	...
	if(N_AL_DT_DL == SN_CT_AL && N_AL_DT_DL_TT == 2)
	{
		...
		else
		{
			...
			{
				(thang_am) = T_AL_DT_DL==12?1:(T_AL_DT_DL + 1);
			} 
		}
	}
	else
	{
		(ngay_am) = ngay + N_AL_DT_DL - 1;
		if((ngay_am)<= SN_CT_AL)
		{
			(thang_am) = T_AL_DT_DL;
		}
		else
		{
			...
			(thang_am) = T_AL_DT_DL + 1 - TN_B_THT;
			if((thang_am) == 13) (thang_am) = 1;
		}
	}
...
//======================================== 
void Solar2Lunar2(unsigned char SolarDay, unsigned char SolarMonth, unsigned int SolarYear,
...
{ 	
	...
	tro_bag_lunar(LUNAR_CALENDAR_LOOKUP_TABLE[(SolarYear-BEGINNING_YEAR)*12+SolarMonth -1]);
	...
	tro_bag_lunar(LUNAR_CALENDAR_LOOKUP_TABLE[(SolarYear-BEGINNING_YEAR)*12+SolarMonth]);
